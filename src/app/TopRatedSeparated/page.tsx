import { getTopRatedMoviesByPage } from "@/lib/api";
import {
  Pagination,
  PaginationContent,
  PaginationEllipsis,
...
export default async function SeperatedTopRated({ searchParams }: PageProps) {
  const { page } = await searchParams;
  const currentPage = Number(page) || 1;
  const data = await getTopRatedMoviesByPage(currentPage);
